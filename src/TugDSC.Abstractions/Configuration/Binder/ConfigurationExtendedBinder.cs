...
namespace TugDSC.Configuration.Binder
{
    /// <summary>
    /// Static helper class that allows binding strongly typed objects to configuration values.
    /// </summary>
    /// <remarks>
    /// This is the same exact code stolen^H^H^H^H^H^H^H borrowed from
    /// <c>aspnet/Configuration/src/Config.Binder</c>, then altered to add some enhancements,
    /// like support for binding to instance properties whose type is an interface, abstract
    /// class or a subtype of a target type.
    /// </remarks>
    public static class ConfigurationExtendedBinder
    {
        /// <summary>
        ...
        public static T GetExt<T>(this IConfiguration configuration)
        {
            ...
            var result = configuration.GetExt(typeof(T));
            if (result == null)
            {
                return default(T);
            }
        ...
        public static object GetExt(this IConfiguration configuration, Type type)
        {
            if (configuration == null)
            {
                throw new ArgumentNullException(nameof(configuration));
        ...
        public static void BindExt(this IConfiguration configuration, string key, object instance)
            => configuration.GetSection(key).BindExt(instance);
        ...
        /// <param name="configuration">The configuration instance to bind.</param>
        /// <param name="instance">The object to bind.</param>
        public static void BindExt(this IConfiguration configuration, object instance)
        {
            if (configuration == null)
            {
                throw new ArgumentNullException(nameof(configuration));
        ...
        public static T GetValueExt<T>(this IConfiguration configuration, string key)
        {
            return GetValueExt(configuration, key, default(T));
        }
        ...
        /// <returns>The converted value.</returns>
        public static T GetValueExt<T>(this IConfiguration configuration, string key, T defaultValue)
        {
            return (T)GetValueExt(configuration, typeof(T), key, defaultValue);
        }
        ...
        /// <param name="key">The key of the configuration section's value to convert.</param>
        /// <returns>The converted value.</returns>
        public static object GetValueExt(this IConfiguration configuration, Type type, string key)
        {
            return GetValueExt(configuration, type, key, defaultValue: null);
        }
        ...
        /// <param name="defaultValue">The default value to use if no value is found.</param>
        /// <returns>The converted value.</returns>
        public static object GetValueExt(this IConfiguration configuration, Type type, string key, object defaultValue)
        {
        ...
        private static object BindInstance(Type type, object instance, IConfiguration config)
        {
            ...
            if (config != null && config.GetChildren().Any())
            {
                ...
                if (instance == null)
                {
                    ...
                    {
                        var newType = Type.GetType(typeName, true);
                        if (!type.IsAssignableFrom(newType))
                        {
                            throw new InvalidOperationException(Resources.FormatError_TypeNotAssignableToTargetType(newType, type));
                        }
                        type = newType;
                    }
