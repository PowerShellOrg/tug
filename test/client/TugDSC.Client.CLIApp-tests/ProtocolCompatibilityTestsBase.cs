...
namespace TugDSC.Client.CLIApp
{
    public class ProtocolCompatibilityTestsBase
    {
        ...
        // the tests that can be overridden using ENV config
        // values by using the format:  TSTCFG_cfg_prop=value
        ...
        protected static void ClassInit(TestContext ctx)
        {
            ...
            TugDSC.Client.AppLog.Factory.AddConsole(LogLevel.Information);

            // Bind optional configuration overrides from ENV to our test config
